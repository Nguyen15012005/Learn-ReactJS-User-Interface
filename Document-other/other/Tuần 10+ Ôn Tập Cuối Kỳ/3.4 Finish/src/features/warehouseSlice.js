...
const warehouseSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(removeWarehouse.fulfilled, (state) => {
        state.loading = false;
      })
...
export const removeWarehouse = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    return await fetch(`http://localhost:3001/warehouses/${id}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
      },
...
// updaateWarehouse({thông tin warehouse})
export const updateWarehouse = createAsyncThunk(
  "warehouse/updateWarehouse",
  async (warehouseToUpdate, { rejectWithValue }) => {
    return await fetch(
      `http://localhost:3001/warehouses/${warehouseToUpdate.id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify(warehouseToUpdate),
      }
    ).catch((err) => rejectWithValue(err));
  }
);
